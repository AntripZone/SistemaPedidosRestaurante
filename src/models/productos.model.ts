...
export const ProductsModel = {
  ...
  // POST 
  create: async (
    dato: CreateProductoInput): Promise<Producto> => {
    const { nombre, categoria, precio, disponible } = dato;
    const query = ` INSERT INTO productos (nombre, categoria, precio, disponible)
      VALUES
        ($1, $2, $3, $4)
      RETURNING *;
    `;
    const { rows } = await pool.query<Producto>(query, [nombre, categoria, precio, disponible]);
    if (!rows[0]) {
  ...
  // PUT
  update: async (id: number, dato: UpdateProductoInput): Promise<Producto | null> => {
